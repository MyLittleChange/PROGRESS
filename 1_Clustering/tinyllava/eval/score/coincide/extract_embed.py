...
class LazySupervisedDataset_w_split(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        # np.save(os.path.join('/home/mila/c/chandhos/COINCIDE_code/COINCIDE_train/playground/data/LLaVA-Instruction', 'random_indices_ours.npy'), random_indices)
        # print('Saved random indices now')
...
def eval_model(args):
    ...
    output_file = os.path.join(args.score_path, f'tan_act_{target_layer_str}_msa_{args.chunk_idx}')
    
    # Resume from existing progress if available
    if os.path.exists(output_file+'.npy'):
        print(f"Chunk {args.chunk_idx} already processed. Skipping...")
        return
    ...
    start_idx = 0
    print('Resume file is ', temp_file)
    print('Resume file exists', os.path.exists(temp_file+'.npz'))
    if os.path.exists(temp_file+'.npz'):
        print(f"Resuming from temporary file: {temp_file}")
        temp_data = np.load(temp_file+'.npz')
        msa_task_emb = temp_data['embeddings']
        start_idx = temp_data['last_idx']
        print(f"Resuming from index {start_idx}")

    # Process batches with resume capability
    for batch_i, batch in enumerate(tqdm(data_loader, total=len(data_loader))):
        if batch_i < start_idx:
            continue
        ...
        print(batch_i)
        # Save progress periodically (e.g., every 100 batches)
        if batch_i % 1000 == 0:
            np.savez(temp_file, embeddings=msa_task_emb, last_idx=batch_i)
            print('loaded temp file')
            np.load(temp_file+'.npz')
    print('done')
    # Save final results and clean up
    msa_task_emb[np.isnan(msa_task_emb)] = 0
    np.save(output_file, msa_task_emb)
    # if os.path.exists(temp_file):
    #     os.remove(temp_file)
            
    # except Exception as e:
    #     # Save progress on error
    #     np.save(temp_file, {'embeddings': msa_task_emb, 'last_idx': batch_idx})
    #     print(f"Error occurred. Progress saved to {temp_file}")
    #     raise e
